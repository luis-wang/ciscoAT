...
def inactive_list(isoprofile, type):
    cucmssh, rst = Login_ssh(isoprofile, type)
    cucmssh.expect("admin:")
...
def Run_upgrade_command(server, isoprofile, type):
    ...
    try:
        ...
        print '5-----------sent [utils system upgrade initiate]-------------------'
...
def Run_Switch_command(server, isoprofile, type): 
    ...
    try:    
        ...
        while True:
            index = cucmssh.expect(["Enter \(yes/no\)?", "Waiting", pexpect.EOF, pexpect.TIMEOUT] )
            if index == 0:
...
#run: python autotest.py /home/work/ISO/UCSInstall_UCOS_8.5.1.16109-1.sgn.iso
if __name__ == '__main__':
    ...
    try:
        ...
        USER        =     ISOprofile[2]
        PASSWORD    =     ISOprofile[3]
        DATASTORE   =     ISOprofile[4]
        DATACENTER  =     ISOprofile[5]
        MD5         =     ISOprofile[6]
        PUBCUCMVM   =     ISOprofile[7]
        
        PUBCUCMIP   =     ISOprofile[8]
        PUBCUCMUSER =     ISOprofile[9]
        PUBCUCMPWD  =     ISOprofile[10]
        SUBCUCMVM   =     ISOprofile[11]
        SUBCUCMIP   =     ISOprofile[12]
        SUBCUCMUSER =     ISOprofile[13]
        SUBCUCMPWD  =     ISOprofile[14]
        ...
        if server.is_connected():
            ...
            pub_vm = cucmweb.Cucmweb(PUBCUCMIP)
            pub_vm.upgrade() 
            inactive_list(ISOprofile, "PUB")
            pub_vm.clearData()                      
            
            logging.info('+++++++++++++++++++++++++++++++++ Run_upgrade_command() PUB successful ,and closed the window+++++++++++++++++++++++++++++')
            ...
            #5                  
            sub_vm = cucmweb.Cucmweb(SUBCUCMIP)
            sub_vm.upgrade()
            inactive_list(ISOprofile, "SUB")          
            sub_vm.clearData()   
            

            logging.info('+++++++++++++++++++++++++++++++++ Run_upgrade_command() SUB successful ,and closed the window +++++++++++++++++++++++++++++++++')
            logging.info('+++++++++++++++++++++++++++++++++ start to switch pub ++++++++++++++++++++++++++++++++++++++++++++++++++')
            
            print '+++++++++++++++++++++++++++++++++ start to switch pub ++++++++++++++++++++++++++++++++++++++++++++++++++'
            ...
            print '+++++++++++++++++++++++++++++++++  start to switch sub ++++++++++++++++++++++++++++++++++++++++++++++++++'     
            Run_Switch_command(server, ISOprofile, "SUB")

            
            print '+++++++++++++++++++++++++++++++++  start to insert and delete phone ++++++++++++++++++++++++++++++++++++++++++++++++++'
            tp = TestPhone(PUBCUCMIP)
            tp.insertPhone()
            tp.deletePhones()
            print '+++++++++++++++++++++++++++++++++  end insert and delete phone ++++++++++++++++++++++++++++++++++++++++++++++++++'
            ...
            #8
            print '\n'*2
            print '*********last step,run command: utils dbreplication runtimestate***********'
            print '\n'*2  
            
            logging.info('\n++++++++++++++last step,run command: utils dbreplication runtimestate+++++++++++++++++')
